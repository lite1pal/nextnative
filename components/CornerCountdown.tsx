...
function TimeCell({ value, label }: { value: number; label: string }) {
  return (
    <div className="flex min-w-[54px] flex-col rounded-2xl bg-black px-2 py-2 text-center text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]">
      ...
      <span className="mt-1 text-[10px] font-semibold tracking-[0.22em] text-white uppercase">
        {label}
      </span>
    </div>
  );
...
export default function CornerCountdown() {
  ...
  return (
    <button
      type="button"
      onClick={handleClick}
      className="fixed right-3 bottom-3 z-50 w-[200px] rotate-[2deg] cursor-pointer rounded-[26px] border border-black bg-emerald-100 p-3 text-left text-black shadow-[0_16px_40px_rgba(0,0,0,0.18)] transition-transform duration-200 hover:translate-y-[-2px] hover:rotate-0 sm:right-5 sm:bottom-5 sm:w-[260px] sm:p-4"
    ...
    >
      <div>
        ...
        <div className="mt-3 text-sm leading-tight font-semibold sm:text-lg">
          $79 pricing ends soon
        </div>
      </div>

      <div className="mt-3 flex gap-2">
        <TimeCell value={timeLeft.hours} label="Hour" />
